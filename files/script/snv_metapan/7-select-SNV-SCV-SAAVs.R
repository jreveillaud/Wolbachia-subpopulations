...
for(i in sample){
  ...
  df <- read.table(paste0("intra-samples/variability-", i, "-filtered.txt"), header=TRUE) # filtered SNVs (intra-samples)
  
  # reads filtered SCV table
  df2 <- read.table(paste0("SCVs/SCV_variability_", i, "_intra-samples_filtered.txt"), header=TRUE) # filtered SCVs (intra-samples)
  ...
  # write table
  write.table(df3, paste0("SCVs/SCV_variability_", i, "_intra-samples_filtered_from_filtered_SNVs.txt"), sep="\t", quote=FALSE, row.names=FALSE)
...
for(i in sample){
  ...
  df3 <- read.table(paste0("SAAVs/SAAV_variability_", i, "_intra-samples_filtered.txt"), header=TRUE) # filtered SAAVs (intra-samples)
  ...
  write.table(df4, paste0("SAAVs/SAAV_variability_", i, "_intra-samples_filtered_from_filtered_SCVs.txt"), sep="\t", quote=FALSE, row.names=FALSE)
